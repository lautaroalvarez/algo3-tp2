...
labelx = "Cantidad de salas"
labely = "Tiempo de ejecucion (ms)"
titulo = "Cantidad de salas vs Tiempo de ejecucion"
...
for row in entrada_normal:
	if salas_actual!=row[2]:
		if salas_actual!=-1:
			datos_actual = np.delete(datos_actual, np.argmax(datos_actual))
			datos_actual = np.delete(datos_actual, np.argmin(datos_actual))
			ejex = np.append(ejex, np.array([salas_actual]), axis=0)
			if datos_actual[np.argmax(datos_actual)] > maximo:
				maximo = datos_actual[np.argmax(datos_actual)]
			#cotaLog = np.append(cotaLog, np.array([200 + math.log(float(salas_actual))]), axis=0)
			#cotaNLog = np.append(cotaNLog, np.array([200 + float(salas_actual) * math.log(float(salas_actual))]), axis=0)
